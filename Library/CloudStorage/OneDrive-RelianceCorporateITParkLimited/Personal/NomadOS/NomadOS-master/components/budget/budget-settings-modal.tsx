...
type TravelStyle = 'budget' | 'midrange' | 'premium';

export function BudgetSettingsModal({ tripId, currentSettings, open, onOpenChange }: BudgetSettingsModalProps) {
    const [currency, setCurrency] = useState(currentSettings?.baseCurrency || "INR");
    const [totalCap, setTotalCap] = useState(currentSettings?.totalCap?.toString() || "");
    const [travelStyle, setTravelStyle] = useState<TravelStyle>(currentSettings?.travelStyle || "midrange");
    const [categories, setCategories] = useState<BudgetCategory[]>(currentSettings?.categories || DEFAULT_CATEGORIES);
    const [newCategoryName, setNewCategoryName] = useState("");
    const [loading, setLoading] = useState(false);

    // Sync local state when currentSettings changes (e.g., after prediction is applied)
    useEffect(() => {
        if (currentSettings) {
            setCurrency(currentSettings.baseCurrency || "INR");
            setTotalCap(currentSettings.totalCap?.toString() || "");
            setTravelStyle(currentSettings.travelStyle || "midrange");
            setCategories(currentSettings.categories || DEFAULT_CATEGORIES);
        }
    }, [currentSettings]);
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <button
                                ...
                                className={`flex flex-col items-center p-3 rounded-lg border-2 transition-all ${travelStyle === 'budget'
                            ...
                            <button
                                ...
                                className={`flex flex-col items-center p-3 rounded-lg border-2 transition-all ${travelStyle === 'midrange'
                                    ? 'border-primary bg-primary/5'
                                    : 'border-muted hover:border-muted-foreground/50'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex flex-col items-center p-3 rounded-lg border-2 transition-all ${travelStyle === 'premium'
                                    ? 'border-primary bg-primary/5'
                                    : 'border-muted hover:border-muted-foreground/50'
                                    }`}
                            >
                        ...
                        <p className="text-xs text-muted-foreground">
                            Used for AI budget predictions. Affects accommodation, dining, and activity estimates.
                        </p>
                    </div>
